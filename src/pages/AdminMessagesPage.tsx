...
export default function AdminMessagesPage() {
  ...
  const allUsers = useMemo(() => getUsers().filter(u => u.role === 'user'), []);

  const handleSend = () => {
    if (!toUserId || !subject || !body || !user) return;
    const toUser = getUsers().find(u => u.id === toUserId);
    sendMessage({ fromUserId: user.id, fromName: user.name, toUserId, toName: toUser?.name || 'User', subject, body });
    setSubject(''); setBody(''); setToUserId(''); setTab('sent');
    ...
    <DashboardLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-foreground">Messages</h1><p className="text-sm text-muted-foreground">Direct messages with users</p></div>
          <Button size="sm" onClick={() => setTab('compose')} className="gap-1"><Mail className="h-3.5 w-3.5" /> Compose</Button>
        </div>
        ...
        {tab === 'compose' && (
          <div className="glass rounded-xl p-5 space-y-3 animate-slide-up">
            <Select value={toUserId} onValueChange={setToUserId}><SelectTrigger className="bg-secondary border-border text-sm"><SelectValue placeholder="Select recipient..." /></SelectTrigger><SelectContent>{allUsers.slice(0, 20).map(u => <SelectItem key={u.id} value={u.id}>{u.name} ({u.email})</SelectItem>)}</SelectContent></Select>
            ...
            <Button size="sm" onClick={handleSend} className="gap-1"><Send className="h-3.5 w-3.5" /> Send Message</Button>
          </div>
        )}

        {tab === 'inbox' && (
          <div className="space-y-2">
            {inbox.length === 0 && <div className="glass rounded-xl p-12 text-center"><Inbox className="h-8 w-8 text-muted-foreground mx-auto mb-3" /><p className="text-muted-foreground">No messages</p></div>}
            {inbox.map(m => (
              <div key={m.id} className={`glass rounded-xl p-4 cursor-pointer transition-colors ${!m.read ? 'border-primary/20' : 'opacity-80'}`} onClick={() => markMessageRead(m.id)}>
                ...
                <p className="text-xs text-muted-foreground ml-5 mt-1 line-clamp-2">{m.body}</p>
              </div>
            ))}
          </div>
        )}
